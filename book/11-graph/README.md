...
* [그래프 최단 경로 구하기](#그래프-최단-경로-구하기)
  * [다익스트라 알고리즘](#다익스트라-알고리즘)
    * [다익스트라 알고리즘 과정](#다익스트라-알고리즘-과정)
  * [벨만-포드 알고리즘](#벨만-포드-알고리즘)
    * [벨만-포드 알고리즘 과정](#벨만-포드-알고리즘-과정)
...
* **음의 가중치가 없는 가중치 그래프**에서 **최단 경로**를 구하는 알고리즘
* 다익스트라 알고리즘은 **그리디적으로 동작**

#### 다익스트라 알고리즘 과정

1. 시작 정점을 설정하고 시작 정점으로부터 특정 정점까지의 **최소 비용을 저장할 공간**과 **직전 정점을 저장할 공간**을 마련
    * 최소 비용을 저장할 공간과 직전 정점을 저장할 공간은 모두 **무한대**(INF)로 초기화
    * 시작 정점의 최소 비용은 0, 직전 정점은 자신으로 저장
2. 아직 **방문하지 않은 정점 중, 현재까지 구한 최소 비용이 가장 적은 정점**을 선택
    * **해당 정점을 거쳐서 특정 정점까지 가는 최소 비용과 특정 정점의 현재까지 구한 최소 비용을 비교하여 작은 값**을 최소 비용으로 갱신
    * 이때 직전 정점도 같이 갱신
3. 정점 개수에서 1을 뺀 만큼 반복
...
1. 시작 정점을 설정한 다음 시작 정점의 최소 비용은 0, 나머지 정점은 INF로 초기화한다. 이후 최소 비용을 갱신할 때 직전 정점도 갱신
2. 정점 개수 -1만큼 다음 연산을 반복
    * 시작 정점에서 갈 수 있는 각 정점에 대하여 전체 정점 각각을 거쳐갈 때 현재까지 구한 최소 비용보다 더 적은 최소 비용이 있는지 확인하여 갱신
    * 최소 비용을 갱신할 때, V의 직전 정점 값도 같이 갱신
3. 과정 2의 세부 사항들을 마지막으로 한 번 더 수행하여 갱신되는 최소 비용이 있는지 확인한다. **만약 있다면 음의 순환이 있음을 의미**
